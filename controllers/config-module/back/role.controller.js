...
async function findDocument(req, res){
    ...
    if( format_data.is_valid ){
        ...
        await h_crud.findDocument('Role', backRoleService, { _id: format_data.body_object.id }, req.body.fields, req.body.options).then( async (result_document) => {
        ...
        res.status(400).send( h_response.request( false, format_data.error_fields, 400, 'Error: Validate Data', 'Role fields required not validated' ) );
    }
};
/**
 * 
...
async function listDocuments(req, res){
    ...
    await h_crud.listDocuments('Role', backRoleService, req.body.query, req.body.fields, req.body.options).then( async (result_document) => {
...
async function createDocument(req, res){
    ...
    if( format_data.is_valid ){
        ...
        await h_crud.createDocument('Role', backRoleService, { name: format_data.body_object.name }, format_data.body_object, false).then( async (result_document) => {
...
async function updateDocument(req, res){
    ...
    if( format_data.is_valid ){
        ...
        format_data.body_object.handle = format_data.body_object.name != null ? h_format.slug( format_data.body_object.name ) : null;
        await h_crud.updateDocument('Role', backRoleService, { _id: document_id }, format_data.body_object).then( async (result_document) => {
...
async function deleteDocument(req, res){
    ...
    if( format_data.is_valid ) {
        ...
        await h_crud.deleteDocument('Role', backRoleService, format_data.body_object.id).then( async (result_document) => {
